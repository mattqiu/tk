...
class tb_user_transfer_account_waring extends MY_Model {
     ...
     * @param 编号 $id
     */
    public function del_user_account($id)
    {
        $sql = "delete from user_transfer_account_waring where id = ".$id;
        $this->db->query($sql);
    }
     ...
     * @param 编号 $id
     */
    public function add_user_account_log($id)
    {
        $this->load->model('tb_cash_account_log_x');
        $sql = "select * from user_transfer_account_waring where id = ".$id;
        ...
        if(!empty($query))
        {
            ...
            if (date('Ym', strtotime($query['transfer_time'])) > 201606)
            {
                // 添加2016年6月份之后的转账记录
                $tb_name = get_cash_account_log_table($query['relate_uid'], date('Ym', strtotime($query['transfer_time'])));
                
                $this->tb_cash_account_log_x->add_log_by_datetime($query['relate_uid'], 11, $amount, '', $query['uid'], date('Ym', strtotime($query['transfer_time'])));
            } 
            else 
            {
                // 添加2016年6月份之前的转账记录                
                $this->tb_cash_account_log_x->add_user_account_commsion_log($query['relate_uid'], 11, $amount, '', $query['uid'], date('Ym', strtotime($query['transfer_time'])));
                
            }          
            $this->del_user_account($id);
        }
