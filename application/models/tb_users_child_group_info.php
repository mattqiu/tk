...
class tb_users_child_group_info extends CI_Model{
    ...
    public function replace($data) {   
        ...
        return $this->add_one($group_id, $_data);
    }
    ...
    public function fix_user_sale_rank_up_time($user_id) {
        $this->_fix_down_user_rank_time($user_id);////向下修复时间
    	$this->load->model('tb_users');
       ...
       // $this->_fix_up_user_rank_time($user_id);//向上修复时间
    }
    /**
     * @author: haiya 向下修复职称时间
     * @date: 2017年7月10日
     * @param type $user_id
     */
    private function _fix_down_user_rank_time($user_id) {
        $this->load->model('tb_users');
    	$childrens = $this->tb_users->find_by_parent_id('id', $user_id);
    	if ($childrens) {
            foreach ($childrens as $c) {
                //递归每个子分支
                $this->_fix_down_user_rank_time($c['id']);
            }
        }
        $user = $this->tb_users->getUserInfo($user_id, array('id, parent_id,sale_rank,sale_rank_up_time'));
        if (empty($user)) {
                return false;
        }
        $sale_rank_up_time = $this->_get_real_sale_rank_time($user["id"],$user['sale_rank']);
    ...
    /**
     * @author: haiya 向上修复职称时间
     * @date: 2017年7月10日
     * @param type $user_id
     */
    private function _fix_up_user_rank_time($user_id) {
        $user = $this->tb_users->getUserInfo($user_id, array('id, parent_id,sale_rank,sale_rank_up_time'));
        if ($user && $user['parent_id'] != 0 && $user['parent_id'] != config_item('mem_root_id')) {
            if (empty($user)) {
                    return false;
            }
            $sale_rank_up_time = $this->_get_real_sale_rank_time($user["id"],$user['sale_rank']);
            ...
            $this->_fix_up_user_rank_time($user["parent_id"]);
        }
        //修复完毕
    	return true;
    }
    ...
    private function _get_real_sale_rank_time($user_id, $user_sale_rank) {
    	...
    	if ($user_sale_rank == 0) {
    		...
    		if ($user_sale_rank == 1) {
    			...
    			$this->load->model('tb_users_level_change_log');
                        $this->load->model('tb_users');
                        $childrens = $this->tb_users->find_by_parent_id('id', $user_id);//查询直属下线
    			...
    			$rank_data = $this->tb_users->find_special_one_from_users($user_id,$user_sale_rank, 3, 'sale_rank_up_time');
