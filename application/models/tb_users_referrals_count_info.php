...
class tb_users_referrals_count_info extends MY_Model{
    protected $table_name  = "users_referrals_count_info";
    ...
    public function join_referrals_count($uid,$before_level,$after_level,$id=null){
        ...
        $count_all = $this->get_one('*',array('uid'=>$uid));

        if($count_all){

            $this->db->where('uid',$uid)->set($update_fields_add,"$update_fields_add+1",FALSE)->update('users_referrals_count_info');

            //$data = ["$update_fields_add = $update_fields_add+1"=>null];
            //$aaa = $this->update_one(['uid'=>$uid],$data);
            ...
            if(in_array($after_level,array(1,2,3,5))){ /** 如果不是注册而是升级，把 $update_fields_minus 的的数量 - 1 */
                ...
                //$data=["$update_fields_minus"=>$newCount];
                //$this->update_one(['uid'=>$uid],$data);
            }
        }else{
            ...
            //
            $this->replace(array('uid' => $uid,$update_fields_add=>1));
        }
    }
