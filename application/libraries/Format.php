...
class Format {
	 ...
	 * @author: derrick
	 * @date: 2017年3月22日
	 * @param: @param Srting $string 校验字符串
	 * @param: @return boolean
	 * @reurn: boolean
	 */
	private function _check_is_serialized($string) {
		if (empty ( $string )) {
			return false;
		}
	 ...
	 * @reurn: mixed
	 */
	public function unserialized($string) {
		if ($this->_check_is_serialized($string)) {
			$string = $this->_from_serialize($string);
		}
		return $string;
	}
