...
<div class="search-well">
    <form action="<?php echo base_url('admin/my_tickets'); ?>" class="form-inline" method="GET" style="margin-bottom: 10px;">
...
<div class="well">
    <form method="post" id='form1' action=""  name="listForm">
        <table class="table table-hover">
            ...
            <tr>
                <th></th>
                <?php if((!empty($adminInfo) && in_array($adminInfo['role'],array(0,2))) || $cus_role==2) { ?>
            	...
            	<th>
                    <?php echo lang('id');?>
                    <a href="<?php echo base_url($order_url.'&o_id=asc')?>"><i class="icon-arrow-up"></i></a>
                    <a href="<?php echo base_url($order_url.'&o_id=desc')?>"><i class="icon-arrow-down"></i></a>
                </th>
                <th><?php echo lang('tickets_type'); ?></th>
                <th><?php echo lang('tickets_title'); ?></th>
                <th>
                    <?php echo lang('time'); ?>
                    <a href="<?php echo base_url($order_url.'&o_time=asc')?>"><i class="icon-arrow-up"></i></a>
                    <a href="<?php echo base_url($order_url.'&o_time=desc')?>"><i class="icon-arrow-down"></i></a>
                </th>
        ...
        <?php if ($list){?>
            <?php foreach ($list as $item) { ?>
                <tr class="one_tickets <?php echo $item['id'].'t'; ?>">
                	<td>
                	<?php if(in_array($item['status'],array(0,1))){echo "<a style='color: #0D0' href='#'>".lang('new_tickets')."</a>";}
                    ...
                    <td class="<?php echo $item['id'].'type'; ?>"><?php if(isset(config_item('tickets_problem_type')[$item['type']])){echo lang(config_item('tickets_problem_type')[$item['type']]);} ?>
						<?php if($item['is_attach']){ ?>
							<img src="<?php echo base_url('img/huixing.png')?>" width="18px" >
						<?php }else{echo "&nbsp;&nbsp;&nbsp;";}?>
					</td>
                    <td class="tips_show"><a href="javascript:view_or_reply_tickets(<?php echo $item['id']?>);"><?php echo mb_substr($item['title'],0,30,'utf-8');if(mb_strlen($item['title'],'utf8')>30){echo '...';}if(!$item['title']){echo 'TPS';} ?></a></td>
                    ...
                    <td width="15%">
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <?php if($item['status']!=4 && $item['status']!=5 && $item['status']!=6){ ?>
                                <li><a class="btn_enable" href='javascript:close_ticket(<?php echo $item['id'];?>);'><?php echo lang('apply_close');?></a></li>
                                <?php }?>
...
function close_ticket(id){
	...
	$.ajax({
		type:"post",
		url:"<?php echo base_url('admin/my_tickets/close_tickets') ?>",
		data:{id:id},
		dataType:"json",
		async:true,
...
function view_tickets_log(id){
	layer.open({
			...
			content: '<?php echo base_url('admin/my_tickets/view_tickets_log'); ?>/' + id,
...
function view_or_reply_tickets(id){
	layer.open({
			...
			content: '<?php echo base_url('admin/my_tickets/view_or_reply_tickets'); ?>/' + id,
            end:function(index){
                //window.location.reload();
...
function get_tickets_status(id){
    $.ajax({
        type:"post",
        url:"<?php echo base_url('admin/my_tickets/get_tickets_status') ?>",
        data:{id:id},
        dataType:"json",
        async:true,
        success:function(res){
...
function return_batch_transfer(){
    ...
    var action = '/admin/my_tickets/batch_transfer';
