...
<div class="well">
	...
	<?php endif; ?>
	<form class="form_ajax"  method="post" action="<?php echo base_url('admin/export_bank_orders/report_to_usa') ?>">
...
<!--                <input class="Wdate span2" id="end_date" type="text" name="end_date" onClick="WdatePicker({lang: '<?php echo $curLanguage; ?>'});" placeholder="<?php echo lang('end_date'); ?>"><br>
		...
		&nbsp;&nbsp;&nbsp;
                <b><?php echo lang('export_bank_time_period')?>:</b>
                <select class="input-medium"  style="width:auto;"  name="time_period" id="time_period">
                    <option value="1"><?php echo lang('time_period2') ?></option>
                    <option value="0"><?php echo lang('time_period1') ?></option>
                </select>
                <br>
...
<script>
	...
	function cleanSubmitBtn(){
        ...
        function date_3(){
            ...
            $.ajax({
                type: "POST",
                url: "/admin/export_bank_orders/export_status_ajax",
                dataType: "json",
...
//                                "<span class='delete' title='删除' style='display:"+status_del+"' alt='删除'><i class='icon-trash'></i>&nbsp;</span>"+
        ...
        $(document).on("click",".download",function(){
            ...
            if(id){
                $.ajax({type: "POST",url: "/admin/export_bank_orders/download_change_s",data:{id:id}});
                window.open("/admin/export_bank_orders/download_file?download_id="+id);
            }else{
                layer.alert("文件为不存在");
            }
        });
        $(document).on("click",".delete",function(){
            var id = $(this).parent().attr("data-id");
            if(id){
                $.ajax({type: "POST",url: "/admin/export_bank_orders/del_change_s",data:{id:id}});
            }else{
                layer.alert("文件为空");
            }
        });
        $(".submit_btn").click(function(){
            ...
            $.ajax({
                type: "POST",
                url: "/admin/export_bank_orders/report_to_usa",
                data:data,
                dataType: "json",
                success: function (res) {
                    if (res.success == 1) {
