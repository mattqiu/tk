...
<form class="form-list2" method="GET">
    <div class="search-well">
        ...
        <input type="text" name="card_number" value="<?php echo $searchData['card_number']; ?>" class="input-medium span2" placeholder="<?php echo lang('paypal_account_t') ?>">
        ...
        <label>
            ...
            <?php if(in_array($adminInfo['role'],array(4)) || in_array($adminInfo['id'],array(1,99,18,68,198,144,280)) ){?>
            <button class="btn" onclick="generate_batch();" type="button"><?php echo lang('generate_batch') ?></button>
            <?php }?>
            <a class="btn" href="<?php echo base_url('/admin/withdraw_table_batch') ?>" ><?php echo lang('view_batch') ?></a>
        </label>
        <label>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th>CNY(￥)</th>
                    <th width="5%"><?php echo lang('paypal_account_t'); ?></th>
                    <th><?php echo lang('payee_name'); ?></th>
                    <!--<th width="5%"><?php echo lang('admin_order_remark'); ?></th>-->
...
//第一列不进行排序(索引从0开始) 
    ...
    $('.withdrawal_rate').click(function () {
        ...
        $.post("/admin/withdraw_table/withdrawal_rate", {rate: rate}, function (data) {
            if (data.success) {
                location.reload();
    ...
    function operating_process(id, status, cause) {
        ...
        $.ajax({
            type: "POST",
            url: "/admin/withdraw_table/alone_check_one",
...
//                    layer.msg('<?php echo lang("result_ok"); ?>');
    ...
    function exportWithdrawal() {
        $('.form-list2').attr('action', "<?php echo base_url('/admin/withdraw_table/exportWithdrawal'); ?>");
        $('.form-list2').submit();
    ...
    function generate_batch() {
        ...
        $.ajax({
            type: "POST",
            url: "/admin/withdraw_table/only_name",
            data: '',
            dataType: "json",
            success: function (res) {
                if (res.success) {
    ...
    $(".confirm").click(function () {
        ...
        $.ajax({
            type: "POST",
            url: "/admin/withdraw_table/generate_batch",
            ...
            success: function (res) {
                if (res.success) {
                    ...
                    layer.msg(res.msg);
                    setTimeout("location = '<?php echo base_url('/admin/withdraw_table_batch'); ?>'", 0);
                } else if (res.msg == 8) {
