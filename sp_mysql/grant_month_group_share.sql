...
set s_month = date_sub(date_sub(date_format(now(),'%y-%m-%d'),interval extract(day from now())-1 day),interval 1 month);
set e_month = date_sub(date_sub(date_format(now(),'%y-%m-%d'),interval extract(day from now())-1 day),interval 0 month);



#统计上个月的总利润额，每种权重的金额
select round(if(sum(order_profit_usd) is not null,sum(order_profit_usd),0)*100) into totalProfit_wh from mall_orders 
where create_time>= s_month and create_time < e_month;

select round(if(sum(order_profit_usd) is not null,sum(order_profit_usd),0)*100) into totalProfit_1di from 
one_direct_orders where create_time>=s_month and create_time < e_month;

select if(sum(order_profit_usd) is not null,sum(order_profit_usd),0) into totalProfit_tps from trade_orders 
where pay_time>=s_month and pay_time<e_month and order_prop in('0','2') and `status` in('3','4','5','6');

set totalProfit = round((totalProfit_wh+totalProfit_1di+totalProfit_tps)*0.3);
