								...
								<?php foreach ($address_list as $k => $v): ?>
									<li<?php if ($v['is_default']): ?> class="selected"<?php endif; ?>
										...
										<div class="address-info">
											<a href="javascript:showBg();" class="modify c-b" onclick="click_addr_edit('<?php echo $k; ?>');">
												<?php echo lang('checkout_deliver_modify'); ?>
											</a>
											<a href="javascript:void(0);" class="shanchu c-b" onclick="submit_delete_addr('<?php echo $v['id']; ?>');">
												<?php echo lang('checkout_deliver_delete'); ?>
											</a>
											<label>
												<?php echo $v['addr_str']; ?>
											...
											<?php else: ?>
												<a href="javascript:void(0);" class="set-default c-b" onclick="submit_set_default_addr('<?php echo $v['id']; ?>');">
						...
						<div class="box-bd">
							...
							<div class="zongs clear">
								...
								<div class="right clear">
									...
									<dl class="clear bk bottom_15">
										<p style="color: #4cae4c"><b><?php echo lang_attr('you_coupons_total_money',$coupons_list)?></b><b>(<?php echo lang("system_auto_clear")?>)</b></p>
									</dl>
									<dl class="clear">
...
<div class="xm-edit-addr wodl BOX_nav clear" id="BOX_nav">
	...
	<div class="item clear">
		<dl>
			<dt><span>&nbsp;</span></dt>
			<dd>
                <textarea type="text" class="xxidz" id="box_addr_detail" maxlength="255" placeholder="<?php echo lang('checkout_addr_detail_placeholder'); ?>"></textarea>
			</dd>
		</dl>
	</div>
...
<script>
			...
			success: function(data) {
				if (data.code == 0) {
					...
					if (data.msg.indexOf("consignee") != -1) {
						...
						layer.msg(err_lang_map.customs_clearance);
					} else {
						console.log(data);
					}
				}
	...
	function submit_choose() {
		...
		data.goods_amount_usd = <?php echo $total_money * 100; ?>;
		...
		$.ajax({
			url: '/use_coupons_submit/do_checkout_order',
			type: "POST",
			data: data,
			dataType: "json",
			success: function(data) {
				...
				if (data.code == 0) {
					...
					layer.msg(data.msg);
					window.location = "/ucenter/my_other_orders";
				} else {
					console.log(data);
					layer.msg(data.msg);
					$('.btn_hong').attr("disabled", false);
