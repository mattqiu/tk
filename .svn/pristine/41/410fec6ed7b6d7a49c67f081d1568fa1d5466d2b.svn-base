...
<div class="search-well">
    <form class="form-inline" method="GET">
        ...
        <?php if ($status <2) { ?>
            <?php if (in_array($uid,array(1,18,99,3))) { ?>
            <button class="btn submit_alipay" onclick="batch_Alipay('<?php echo $searchData['batch_num']; ?>');" type="button"> <?php echo lang('submit_alipay') ?></button>
            <button class="btn" onclick="cancel_batch(<?php echo $batch_num; ?>)" type="button"> <?php echo lang('cancel_batch') ?></button>
            <?php } ?>
        <?php }else if($status ==2 && in_array($uid,array(1,99,3,18,198))){?>
            <button class="btn" onclick="cancel_batch(<?php echo $batch_num; ?>)" type="button"> <?php echo lang('cancel_batch') ?></button>
        <?php } ?>
    </form>
</div>
                ...
                <?php if ($list) { ?>
                    <?php foreach ($list as $k => $item) { ?>
                        <tr>
                            ...
                            <td width="8%">
                                <?php if (in_array($uid,array(1,18,64,99,68,198))) { ?>
...
//第一列不进行排序(索引从0开始) 
    ...
    function batch_Alipay(id) {
        ...
        $.ajax({
            type: "POST",
            url: "/admin/withdraw_table_batch_detail/batch_Alipay",
            data: {id: id},
            dataType: "json",
    ...
    function cancel_batch(id) {
        layer.confirm('<?php echo lang('double_confirm'); ?>', {icon: 3, title: '<?php echo lang('cancel_confirm'); ?>'}, function (index) {
            $.ajax({
            type: "POST",
            url: "/admin/withdraw_table_batch_detail/generate_batch",
            data: {id: id},
            dataType: "json",
            success: function (res) {
                if (res.success) {
                    layer.msg('<?php echo lang("result_ok"); ?>');
                    setTimeout("location = '<?php echo base_url('/admin/withdraw_table_batch'); ?>'", 3000);
                } else {
                    layer.msg('<?php echo lang("result_false"); ?>');
                }
            }
    ...
    function operating_process(id, status, cause) {
        ...
        $.ajax({
            type: "POST",
            url: "/admin/withdraw_table_batch_detail/alone_check_one",
