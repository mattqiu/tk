...
//        $res = $this->db->select('goods_amount_usd,score_year_month')->from('trade_orders')->where('shopkeeper_id',$uid)->where_in('status',array('1','3','4','5','6','90','97','111'))->where_in('order_prop',array('0','1'))->get()->result_array();
    ...
    /**
     * @author: derrick 获取用户的业绩
     * @date: 2017年7月5日
     * @param: @param int $user_id
     * @reurn: return_type
     */
    public function find_by_user_id($user_id, $page = 1, $size = 12) {
    	return $this->db->where('uid', $user_id)->limit($size, max($page - 1, 0) * $size)->order_by('year_month', 'DESC')->get($this->table_name)->result_array();
    }
