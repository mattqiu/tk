 ...
 *
 */
class pre_week_team_bonus extends MY_Controller {
    ...
    public function index(){
        ...
        $this->_viewData['title'] = lang('pre_week_team_bonus');        
        
        $searchData = $this->input->get()?$this->input->get():array();
        $searchData['page'] = max((int)(isset($searchData['page'])?$searchData['page']:1),1);
       
        //每周团队
        $list = $this->o_month_leader_bonus_option->pre_bonus_list($searchData,1);
        $this->_viewData['list'] = $list;


        $this->load->library('pagination');
        $url = 'admin/pre_week_team_bonus';
        add_params_to_url($url, $searchData);
        $config['base_url'] = base_url($url);
        $config['total_rows'] = $this->o_month_leader_bonus_option->pre_bonus_all($searchData,1);
    ...
    public function get_pre_bonus_state()
    {
         ...
         $ret_value = $this->tb_grant_pre_bonus_state->get_state(25);
         if(!empty($ret_value))
         {
