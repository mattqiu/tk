...
if(isset($rows['reply']) && !empty($rows['reply'])){ ?>
	<div class="well msg_big_box" style="<?php if(($adminInfo['role']!=0 && $adminInfo['role']!=2) || $rows['org']['admin_id'] == 0){echo 'height:750px';}?>">
	...
	<?php if($adminInfo['role']==0 || $adminInfo['role']==2){//客服经理才有权限回复 ?>
	<?php if(!empty($rows['org']) && $rows['org']['admin_id'] != 0){//原始工单不能回复 ?>
	<div class="well input_box">
		<div class="change_c">
			...
			<?php if(check_right('tickets_assign_right')){ ?>
            <span class="select_span">
                ...
                <select class="transfer" name="transfer" style="width: 160px;">
					...
					<?php if(!empty($cus) && isset($cus)){
						foreach ($cus as $val) { ?>
							<option value="<?php echo $val['id'] ?>"><?php echo $val['job_number'].' ('.explode('@',$val['email'])[0].')';?>
							</option>
						<?php } ?>
					<?php }?>
				</select>
...
<script>
	var tid = <?php if(!empty($rows['org'])){ echo $rows['org']['id'];}else{echo 0;} ?>;
	function refresh(){
		$.ajax({
			type: "POST",
	...
	$(function(){
		...
		var id = 0;
		 	id = <?php if(!empty($rows['org'])){ echo $rows['org']['id'];}else{echo 0;} ?>;
		var au = autosize($('textarea'));
		var msg_big_box_obj = $(".msg_big_box");
		var content_obj = $('#content');
		var count_obj 	= $('#count');
		var t_note_content 	= getCookie('t_content_m'+id);
		...
		$('.template_type').change(function(){
			...
			if( $(this).val()!='e'){
				if( $(this).val()!='e'){
					$.ajax({
						type:"post",
						url:"<?php echo base_url('admin/all_tickets/get_template');?>",
						data:{t:val},
						dataType:"json",
						success:function(res){
							if(res.success==1){
		...
		$('.priority').change(function(){
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url('admin/all_tickets/change_tickets_priority'); ?>",
				dataType: "json",
				data:{id:id,p:val},
				success: function (res) {
					if(res.success==1){
		...
		$('.transfer').change(function(){
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url('admin/all_tickets/do_transfer') ?>",
				dataType: "json",
				data:{id:id,c_id:val},
				success: function (res) {
					if(res.success==1){
						layer.msg(res.msg);
						parent.$('.'+id+'t').children('.admin_name').text(txt);
					}else{
						layer.msg(res.msg);
					}
				}
		...
		content_obj.focus().keyup(function(){
			...
			addCookie('t_content_m'+id,content_obj.val(),48);
		});
		...
		$("button[name='submit']").click(function () {
			...
			$.ajax({
				type: "POST",
				url: "/admin/all_tickets/do_reply",
				data: $('#reply_tickets').serialize(),
				dataType: "json",
				success: function (data) {
					if (data.success) {
						layer.msg(data.msg);
						deleteCookie('t_content_m'+id);
						$('#content').val("");
						window.location.reload();
					}else {
