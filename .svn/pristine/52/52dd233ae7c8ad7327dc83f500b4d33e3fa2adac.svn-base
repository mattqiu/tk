...
class CI_DB_active_record extends CI_DB_driver {
	...
	//是否强制使用主库 TRUE是 FALSE否
	var $ar_force_master        = FALSE;

	// --------------------------------------------------------------------

    /**
     * 强制使用主库，目前只支持读操作，即 get 和 count_all_results 方法
     * eg. $this->db->from('table')->where($condition)->force_master()->get();
     * @return $this
     */
    public function force_master(){
        $this->ar_force_master = TRUE;
        return $this;
    }
	/**
	 * Select
	...
	public function getSql(){
        $sql = $this->_compile_select();
        //DRDS提供指定将SQL下发到主实例的HINT
        if ($this->ar_force_master === TRUE) {
            $sql = '/*TDDL:MASTER*/' . $sql;
        }
        return $sql;
    }
