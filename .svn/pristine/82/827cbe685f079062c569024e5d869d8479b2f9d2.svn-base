...
class AlipaySubmit {
	...
	function buildRequestMysign($para_sort) {
		...
		//RSA需要双引号，MD5加密不需要双引号参数值
		$is_flag = strtoupper(trim($this->alipay_config['sign_type'])) == 'RSA' ? true : false ;
		$prestr = createLinkstring($para_sort,$is_flag);
		...
		switch (strtoupper(trim($this->alipay_config['sign_type']))) {
			case "MD5" :
				...
				break;
			case "RSA" :
				$mysign = rsaSign($prestr, APPPATH.'third_party/alipay/rsa_private_key.pem');
				break;
			default :
				$mysign = "";
		}
