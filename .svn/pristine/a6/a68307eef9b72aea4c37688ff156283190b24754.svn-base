<?php
$up_path = realpath('application/third_party/unionpay/');
include_once $up_path . '/SDKConfig.php';
include_once $up_path . '/PublicEncrypte.php';
include_once $up_path . '/common.php';
include_once $up_path . '/log.class.php';
...
//	$cert_path = SDK_SIGN_CERT_PATH;
	...
	$cert_path = APPPATH .'third_party/unionpay/certs/'.$unionpayConfig['SDK_SIGN_CERT_PATH'];
...
//	$log->LogInfo ( '报文去[signature] key=val&串>' . $params_str );
	...
	$params_sha1x16 = sha1 ( $params_str, FALSE );
//	$log->LogInfo ( '[$public_key]' . $public_key );
//	$log->LogInfo ( '[$signature]' . $signature );
//	$log->LogInfo ( '[$params_sha1x16]' . $params_sha1x16 );
...
//	$log = new PhpLog ( SDK_LOG_FILE_PATH, "PRC", SDK_LOG_LEVEL );
//        $log->LogInfo ( '[$certId]' . $certId );
	// 证书目录
    $ci = & get_instance();
	$ci->load->model('m_debug');

//	$cert_dir = SDK_VERIFY_CERT_DIR;
	$cert_dir = APPPATH .'third_party/unionpay/certs/';
//	$log->LogInfo ( '验证签名证书目录 :>' . $cert_dir );
//        $log->LogInfo ( '[$cert_dir]' . $cert_dir );
...
//		$ci->m_debug->log('没有找到证书ID为[' . $certId . ']的证书');

	} else {
//		$log->LogInfo ( '证书目录 ' . $cert_dir . '不正确' );
//		$ci->m_debug->log('证书目录 ' . $cert_dir . '不正确');
	}
	closedir ( $handle );
	return null;
}
...
function getCertId($cert_path) {
	$pkcs12certdata = file_get_contents ( $cert_path );
        ...
        $unionpayConfig = get_unionpay_config();
        $signPwd = $unionpayConfig['SDK_SIGN_CERT_PWD'];
	openssl_pkcs12_read ( $pkcs12certdata, $certs, $signPwd );
	$x509data = $certs ['cert'];
...
function getPrivateKey($cert_path) {
	$pkcs12 = file_get_contents ( $cert_path );
        $unionpayConfig = get_unionpay_config();
        $signPwd = $unionpayConfig['SDK_SIGN_CERT_PWD'];
	openssl_pkcs12_read ( $pkcs12, $certs, $signPwd );
	return $certs ['pkey'];
}
