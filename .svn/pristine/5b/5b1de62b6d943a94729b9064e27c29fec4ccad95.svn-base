...
CREATE  PROCEDURE charge_month_fee(IN curDay INT,IN curMonthLastDay INT)
BEGIN
			/**用于替换grant_comm_single  原因 新的实现了分红表的数据分表*/
	    -- 定义需执行的临时表的最小和最大id

            DECLARE minct,users_m_id,uid_new,maxct,_users_id,monthlyFeeCouponNum,_leftMonthlyFeeCouponNum,fee_num ,record_count,coupon_num_change,total_record INT  DEFAULT 0;
            ...
            IF total_record>0 THEN
							...
							END IF;
							 ...
							 WHILE current_page<page_count DO
								...
								-- coupon_num_change为0 首次使用月费券

								IF (monthlyFeeCouponNum>0 && (coupon_num_change=0|| _country_id<>3 )) THEN
				   ...
				   SET monthFeeOk=FALSE;
										...
										ELSE
											...
											IF _is_auto=1 THEN
													...
													set users_m_id = left(_users_id,4);
													IF users_m_id >=1382 THEN
														set table_name_news =   concat(table_name,"_",users_m_id );							
														SET @sqlstr= CONCAT('insert into ', table_name_news ,'(`uid`,`item_type`,`amount`)values(', CONCAT_WS(',',_users_id,14,ROUND(price_diff)*100),');');   -- 创建表
													ELSE
			  ...
			  END WHILE;
		       ...
		       PREPARE STMT FROM @STMT;
				   ...
				   SET @STMT =CONCAT('UPDATE users SET `status`=1,`store_qualified`=1  WHERE `id` in(',ids,')');
