...
//    		$cate_info[$k]['list']=$this->db_slave->select('title,id')->where('display',1)
//                ->where('language_id',$language_id)->where('cate_id',$v['type_id'])
//                ->order_by('sort','asc')->get('news')->result_array();
            $cate_info[$k]['list']= $this->tb_news->get_list('title,id',
                ['display'=>1,'language_id'=>$language_id,'cate_id'=>$v['type_id']],array(),
                10,0,['sort'=>'asc']);
    	}
...
//    		$cate_info['artical']=$this->db_slave->where('id',$artical_id)->get('news')->row_array();
            $cate_info['artical'] = $this->tb_news->get_one('*',['id'=>$artical_id]);
    	}
    	    ...
    	    if(isset($val['list'])) {
        	    foreach($val['list'] as $art) {
        	        if($artical_id == $art['id']) {
        	            $cate_info['parent_id']=$val['type_id'];
        	            $cate_info['parent_name']=$val['type_name'];
        	        }
        	    }
    	    }
    	}
