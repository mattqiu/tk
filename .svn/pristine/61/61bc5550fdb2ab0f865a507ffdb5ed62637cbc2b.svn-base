...
global $API_UserName,$API_Password,$API_Signature,$API_Endpoint,$version,$subject,$AUTH_token,$AUTH_timestamp,$AUTH_signature;
...
function hash_call($methodName,$nvpStr)
{
   ...
   //Set proxy name to PROXY_HOST and port number to PROXY_PORT in constants.php 
	...
	if (curl_errno($ch)) {
		...
		return array('msg'=>curl_error($ch),'success'=>FALSE);
	 } else {
		 //closing the curl
			curl_close($ch);
	  }
